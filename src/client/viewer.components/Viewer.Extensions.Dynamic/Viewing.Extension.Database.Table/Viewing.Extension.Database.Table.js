...
class DatabaseTableExtension extends MultiModelExtensionBase {
  ...
  buildMaterialMap (model, dbMaterials) {
    ...
    return new Promise(async(resolve, reject) => {
      ...
      //Mapping materials for possible uses
        const volumePropResults =
          await Toolkit.getBulkPropertiesAsync(
            model, componentIds, ['Volume'])

        const volumeResults =
          volumePropResults.map((result) => {
          ...
          if(materialName !== 'undefined') {
            ...
            if (dbMaterial) {
              ...
              if (item) {
                ...
                const volumeProp = find(volumeResults, { dbId })

                const volume = volumeProp ? volumeProp.displayValue : 1.0
  ...
  toUSD (price, currency) {
    ...
    switch (currency) {
      ...
      case 'EUR': return pricef * 1.20;
      case 'USD': return pricef * 1.0;
      case 'JPY': return pricef * 0.009;
  ...
  onUpdateItem (item, externalUpdate) {
    ...
    if (item) {
      ...
      entry.totalCost = entry.totalVolume * this.toUSD(
